...
best_model_path_list=("/project/outputs/aekl_eeg_no-spectral_shhs1_1" "/project/outputs/aekl_eeg_no-spectral_shhs1_3" "/project/outputs/aekl_eeg_no-spectral_shhs1_1" "/project/outputs/aekl_eeg_spectral_3_double_sleep_edfx")
...
for ((i=0; i<${#specs[@]}; i++)); do
  ...
  for ((j=0; j<${#latent_channels[@]}; j++)); do
	  ...
	  runai submit \
	    ...
	    --command -- bash /project/src/bash/start_training.sh python3 /project/src/train_ldm.py num_channels=${channel} spe=${sp} autoencoderkl_config_file_path=${autoencoderkl_config_file_path} best_model_path=${best_model_path} latent_channels=${latent_channel} path_train_ids=${path_train_ids} path_valid_ids=${path_valid_ids}
	done
done
