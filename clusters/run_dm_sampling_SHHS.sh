...
for sp in "${specs[@]}"; do
  for indice in {0..1000..200}; do
    ...
    runai submit \
      --name dm-sample-shhs-${sp}-${begin}-${end} \
      ...
      --command -- bash /project/src/bash/start_training.sh python3 /project/src/sample_trials_ddpm.py spe=${sp} dataset=${dataset} start_seed=${begin} stop_seed=${end}
	done
done
