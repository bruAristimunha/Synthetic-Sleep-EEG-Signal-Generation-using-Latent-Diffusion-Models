...
def main(args):
    ...
    for step, batch in pbar:
        ...
        for batch2 in test_loader_2:
            ...
            ms_ssim_list.append(ms_ssim(torch.from_numpy(img_cropped).to(device), torch.from_numpy(img2_cropped).to(device)).item())
        pbar.update()
    ...
    plt.savefig(f'/project/outputs/{args.dataset}_frequency_{frequency}_test_{i}.pdf')
    with open(f'/project/outputs/{args.dataset}_frequency_{frequency}_test_{i}.pickle', 'wb') as handle:
        pickle.dump(frequency_data, handle)
        
    with open(f'/project/outputs/{args.dataset}_mm_ssim_list_{frequency}_test_{i}.pickle', 'wb') as handle:
        pickle.dump(ms_ssim_list, handle)
